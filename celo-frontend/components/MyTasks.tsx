...
export default function MyTasks({ onToast }: Props) {
  ...
  const created = myCreatedTasks.filter((task) => filter === "all" || task.status === filter);
  ...
  const isEmpty = created.length === 0 && accepted.length === 0 && reviews.length === 0 && payouts.length === 0;

  const sections = [
    { key: "review", title: "Needs Your Review", icon: <IconCheck className="w-4 h-4 text-sky-300" />, data: reviews, tone: "rgba(56,189,248,0.08)" },
    { key: "payout", title: "Ready For Payout", icon: <IconCoin className="w-4 h-4 text-fuchsia-300" />, data: payouts, tone: "rgba(217,70,239,0.08)" },
    { key: "created", title: "Created By You", icon: <IconPlus className="w-4 h-4 text-teal-400" />, data: created, tone: "rgba(20,184,166,0.08)" },
    { key: "accepted", title: "Assigned To You", icon: <IconCheck className="w-4 h-4 text-green-300" />, data: accepted, tone: "rgba(34,197,94,0.08)" },
  ];
                        ...
                        task.status === "in_progress"
                          ...
                          : task.status === "submitted"
                            ...
                            : task.status === "approved"
                              ? "Pay"
                              : "Open",
                      onClick: () => onToast("Open the task details page to continue this workflow.", "info"),
                    }}
                  />
                ))}
              </div>
