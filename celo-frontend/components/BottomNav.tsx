...
export default function BottomNav() {
  ...
  // Right side: swap Profile for Admin when admin
  const navRight = isAdmin
    ? [
      ...
      <div className="flex items-center justify-around h-16" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        {NAV_LEFT.map(({ href, label, Icon }) => (
          <Link
            key={href}
            href={href}
            className={`flex flex-col items-center gap-1 px-4 py-2 transition-colors ${isActive(href) ? "text-teal-400" : "text-slate-500"}`}
          >
        ...
        <div className="flex flex-col items-center -mt-6">
          <Link
            ...
            style={{
              background: "linear-gradient(135deg, #14b8a6, #22c55e, #eab308)",
              boxShadow: "0 0 24px rgba(20,184,166,0.45), 0 4px 16px rgba(0,0,0,0.4)",
            }}
          >
            <IconPlus className="w-7 h-7 text-white" />
          </Link>
          <span className={`text-[10px] font-semibold mt-1 ${isActive("/create-task") ? "text-teal-400" : "text-slate-400"}`}>
            Create
          </span>
        </div>

        {navRight.map(({ href, label, Icon }) => (
          <Link
            key={href}
            href={href}
            className={`flex flex-col items-center gap-1 px-4 py-2 transition-colors ${isActive(href) ? "text-teal-400" : "text-slate-500"}`}
